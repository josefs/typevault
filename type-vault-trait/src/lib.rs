...
pub fn serialize_type<T: VaultType>(value: &T) -> Vec<(Vec<u8>, ValueId)> {
    let mut nested_dest = vec![];
    let mut dest = vec![];
    value.serialize_into(&mut nested_dest, &mut dest);
    let hash = {
...
impl<T: VaultType> VaultType for Box<T> {
    ...
    fn serialize_into(&self, nested_dest: &mut Vec<(Vec<u8>, ValueId)>, dest: &mut Vec<u8>) {
        (**self).serialize_into(nested_dest, dest)
    }

    fn serialize_prefix(&self, fields_in_prefix: u64) -> Vec<u8> {
        (**self).serialize_prefix(fields_in_prefix)
    }
    fn deserialize_value<'a>(data: &'a [u8], lookup_id: &dyn Fn (u64) -> Option<Vec<u8>>) -> Option<(&'a [u8],Self)> where Self: Sized {
...
impl<T: VaultType, U: VaultType> VaultType for (T,U) {
    ...
    fn serialize_into(&self, nested_dest: &mut Vec<(Vec<u8>, ValueId)>, dest: &mut Vec<u8>) {
        self.0.serialize_into(nested_dest, dest);
        self.1.serialize_into(nested_dest, dest);
    }

    fn serialize_prefix(&self, _fields_in_prefix: u64) -> Vec<u8> {
    ...
    fn serialize_into(&self, nested_dest: &mut Vec<(Vec<u8>, ValueId)>, dest: &mut Vec<u8>) {
        match self {
            Some(inner) => {
                ...
                inner.serialize_into(nested_dest, dest);
            },
            None => {
